# Contributing to ep_disable_change_author_name

Thanks for helping improve `ep_disable_change_author_name` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_disable_change_author_name` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_disable_change_author_name from your local clone
pnpm run plugins i --path ../ep_disable_change_author_name

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_disable_change_author_name/` are picked up after a server restart.
...
* Etherpad version, ep_disable_change_author_name version, browser + OS
* Steps to reproduce
